...
```
rodar_tcc_final.bat      REM ← coleta final: build + run (31 reps) + figuras + verificação --strict
rodar_testes.bat         REM script antigo (build + smoke test, sem repetições) — mantido por compatibilidade
```
`rodar_tcc_final.bat` is the canonical end-to-end script: it checks the corpus, builds (MSVC `cl /O2 /std:c++17 /EHsc … OpenCL.lib`, falling back to `g++ -std=c++17 -O2 … -lOpenCL`), runs the sweep into a dedicated dir (`results-csv\final`), gates on the CPU/GPU regression, then generates the figures and runs the `--strict` sanity check. Set `OPENCL_SDK=C:\Program Files (x86)\OCL_SDK_Light` (or equivalent) if headers/libs are not on the default path. Build output: `hc8_opencl_test.exe`. Step-by-step run instructions for a fresh machine live in `INSTRUCOES_TESTES_AMIGO.md`.
...
|---|---|---|
| `--run-examples` | off | Run the 15 scenarios over `bd/*.100MB` (3 bases × m=8/16/32/64/128). First load of each base is also measured `cold`. |
| `--repeat <n>` | 30 | Measured repetitions per scenario (warm). Median + CV% reported. |
| `--warmup <n>` | 1 | Discarded warmup repetitions before measuring. |
...
| `--csv-dir <dir>` | `results-csv` | Where CSVs are written (use a clean dir like `results-csv/final` for the final sweep) |
| `--no-csv` | (csv on) | Disable CSV export |

Each scenario runs `--warmup`+`--repeat` times; correctness (CPU vs GPU `same_total ∧ same_overflow ∧ same_indices`) is checked **every** repetition and any divergence prints `[REGRESSION-FAIL]`. Per-scenario the host computes median + CV% and prints a `[STATS]` line. The aggregate is written to a single `timings_<UTC>.csv` (schema in §4.6); per-index correctness CSVs are written once per scenario under `<csv-dir>/indices/`.

### 3.3 Analysis pipeline

```
source ../.venv/bin/activate          # pandas, matplotlib, seaborn, numpy
python analise_benchmarks.py --input results-csv/final --out-dir figuras-benchmark --strict
```
`--input` must point at the **clean dir of the final run** (default `results-csv/final`); the script reads only the **most recent `timings_*.csv`** there — it does **not** rglob the whole tree or average across batches (that was the bug that halved Table 1). It produces `fig1_speedup.pdf` (speedup with error bars, warm/cold in separate panels), `fig2_tempos_absolutos.pdf` (warm CPU search vs GPU wall, log scale, error bars), `fig3_memoria_gpu.pdf` (decomposition; cold panel shows host-side file_read+upload), plus two Markdown tables (warm with median±CV%, cold with components). `--strict` exits non-zero if any row violates `kernel+h2d+d2h ≤ wall` or `same_indices=0`.
...
- Host reports H2D, kernel, D2H **separately** and a derived `gpu_total = h2d + kernel + d2h`. The thesis's central finding (host-side cost dominates the *cold* regime) depends on this separation — never collapse them into a single wall-clock number.
- `gpu_query_wall` is the per-query wall (`std::chrono` around `run_hc8_gpu_inner`): it includes pattern/F H2D + kernel + D2H but **excludes** the one-time context creation (hoisted in `run_benchmark_examples`) and the text upload. The text upload (`upload_text_pinned`) and the disk read (`read_binary_file`) are timed separately and only counted in the **cold** total.
- **Fair cold comparison (C3):** the *cold* total charges the 100 MB file read to **both** sides — `cpu_total = file_read + preproc + cpu_search_median` vs `gpu_total = file_read + upload_median + gpu_query_wall_median`. Do not revert to charging the read to the GPU only.
- **Invariant:** `kernel_median + h2d + d2h ≤ gpu_query_wall_median`. `analise_benchmarks.py --strict` asserts this; if it ever fails, the aggregation is corrupt (this was the m=8 `kernel > wall` symptom).

### 4.6 CSV schema

**Timings CSV — `timings_<UTC>.csv` (the analysis source).** One row per `(scenario, regime)`; all stats computed
host-side. This is what `analise_benchmarks.py` reads:
`run_tag, text_file, base, pattern_len, regime, repeats, cpu_search_ms_median, cpu_search_ms_cv, cpu_preproc_ms, file_read_ms, gpu_upload_ms_median, gpu_upload_ms_cv, h2d_ms, kernel_ms_median, kernel_ms_cv, d2h_ms, gpu_query_wall_ms_median, gpu_query_wall_ms_cv, cpu_total_ms, gpu_total_ms, speedup, found_total, overflow, same_indices`
- `regime ∈ {cold, warm}`. Warm rows: `file_read = gpu_upload = 0`, `cpu_total = cpu_search_median`, `gpu_total = gpu_query_wall_median`. Cold rows: see §4.5 (fair comparison).
- The Python script does **not** recompute medians or join CPU↔GPU; it just reads and plots. `speedup = cpu_total_ms / gpu_total_ms`.

**Per-index correctness CSVs** (under `<csv-dir>/indices/`, written once per scenario via `write_run_csv`/`write_run_csv_cpu`) keep the legacy per-`(index_order, index_value)` schema and exist only for the `same_indices` audit trail — they are **not** read by the analysis.
...
### 4.9 Benchmark statistics

- Each scenario is measured `--warmup` (default 1, discarded) + `--repeat` (default 30) times. The reported value per metric is the **median**; dispersion is the **CV%** (`compute_stats` in `main.cpp`). Never report a single-shot timing as if it had statistical weight — that was a prior methodology gap.
- **Cold vs warm** is intrinsic to the cache, not a flag: the first time a text is loaded it pays disk read + upload (`cold`); every subsequent search on the cached VRAM buffer is `warm`. Each base yields 1 cold row + 5 warm rows → 3 cold + 15 warm total. The cold scenario also gets a warm row at the same `(base, m)`, enabling a direct cold-vs-warm comparison.
- Keep the GPU otherwise idle during a run (see anti-pattern #14) or CV% inflates and `analise_benchmarks.py` warns (`cv > 15%`).
...
16. **Do not run `analise_benchmarks.py` over the whole `results-csv/` tree, and do not average across run batches.** Point `--input` at the clean run dir (`results-csv/final`); the script reads only the newest `timings_*.csv`. Recursive globbing + mean over `(text_file, pattern_len)` was the C1/C4 bug that halved Table 1 and produced `kernel > wall`.
17. **Do not drop the `kernel+h2d+d2h ≤ gpu_query_wall` invariant or the `--strict` gate.** If `--strict` fails, fix the data/aggregation — do not loosen the assert. Likewise keep `[REGRESSION-FAIL]` gating the run: a divergent `same_indices` invalidates the collection.
